 ...
 *
 * <p>Important: An action chosen at timestep t is executed by the simulator after the decision.
 * Therefore, movement progress must be evaluated at the next timestep.
 *
 * <p>This class does not change Police actions.
 */
public final class URFPoliceStuckDetector {
  ...
  /**
   * Evaluates the previous final action using the newly observed position at the current timestep.
   */
  public synchronized void evaluate(URFPoliceMetrics metrics) {
    ...
    boolean confirmed =
        this.consecutiveFailedMoves >= CONFIRMED_STUCK_AFTER_FAILED_MOVES
  ...
  /** Determines whether the previous MOVE actually produced movement progress. */
  private void evaluatePreviousActionOutcome(URFPoliceMetrics metrics) {
     ...
     */
    if (this.previousActionTarget != null
        && this.previousActionTarget.equals(this.lastFailedMoveTarget)) {
  ...
  /** Saves the current final action for evaluation during the next timestep. */
  ...
  /** Resolves the logical target associated with the action source. */
  ...
  /** Structured fields appended to the existing URF_POLICE_METRICS runtime line. */
  public synchronized String toLogFields() {
    ...
    return "URF_STUCK"
        ...
        + " failedMoveStreak="
        + this.consecutiveFailedMoves
        + " sameTargetFailedMoveStreak="
        + this.consecutiveSameTargetFailedMoves
        + " totalFailedMoveOutcomes="
        + this.totalFailedMoveOutcomes
        + " possibleStuckEvents="
        + this.possibleStuckEvents
        ...
        + " trackedActionSource="
        + this.previousActionSource
        + " trackedActionTarget="
        + idValue(this.previousActionTarget)
        + " trackedPathLength="
