...
import urf.police.observation.URFPoliceCsvExporter;
import urf.police.observation.URFPoliceMetrics;
import urf.police.observation.URFPoliceStuckDetector;
 ...
 * <p>PI-1 design goals:
 *
 * <p>1. Do not inherit DefaultExtActionClear. 2. Do not inherit Sample decision logic. 3. Clear
 * blockades on the Police agent's current Road. 4. Never generate ActionMove. 5. Return null when
 * clearing is not appropriate so that DefaultTacticsPoliceForce can continue to ExtActionMove. 6.
 * Detect stagnant clearing using blockade repair cost. 7. Use deterministic directional clearing as
 * recovery. 8. Preserve the trusted URF observation/CSV pipeline.
 *
 * <p>IMPORTANT:
 *
 * <p>This PI-1 module intentionally does NOT perform:
 *
 * <p>- target selection, - global path planning, - Fire/Ambulance prioritisation, - multi-Police
 * coordination, - exploration.
 *
 * <p>Those responsibilities belong to later independent URF modules.
 */
public final class URFPoliceExtActionClear extends ExtAction {

  /**
   * Number of consecutive clear attempts with no observed reduction in blockade repair cost before
   * changing the clearing method.
   */
  private static final int STAGNANT_CLEAR_LIMIT = 3;

  /**
   * Extra distance placed beyond the nearest blockade boundary during directional recovery.
   *
   * <p>RCRS coordinates are expressed in world distance units.
   */
   ...
   *
   * <p>PI-1 does not perform target selection itself.
   */
  ...
  /**
   * Number of consecutive attempts for which the same blockade did not show repair-cost
   * improvement.
   */
   ...
   *
   * <p>Target selection remains outside this class.
   */
  @Override
   ...
   * <p>PI-1 rule:
   *
   * <p>This method may return:
   *
   * <p>ActionClear null
   *
   * <p>It deliberately never returns ActionMove.
   */
  @Override
  public ExtAction calc() {
    ...
    this.metrics.recordClearModuleResult(
        this.agentInfo.getTime(), this.target, this.result, elapsedNanos);
    ...
    if (this.result != null) {
      ...
      System.out.println(
          this.metrics.toLogLine()
              + " "
              + this.stuckDetector.toLogFields()
              + " pi1ClearDecision="
              + decision
              + " recoveryClearCount="
              + this.recoveryClearCount);
  ...
  /** Find the nearest valid blockade on the Police agent's current Road. */
   ...
   *
   * <p>Distance is measured from the Police location to the nearest point on the blockade polygon.
   */
   ...
   *
   * <p>Primary ordering: nearest physical blockade.
   *
   * <p>Tie-break: smaller EntityID for deterministic behaviour.
   */
  ...
  /**
   * Compare the currently observed blockade repair cost with the previous clear attempt.
   *
   * <p>A decreasing repair cost means clearing is progressing.
   *
   * <p>An unchanged or increased repair cost counts as stagnant.
   */
  private void updateClearProgress(Blockade blockade) {
    ...
    if (this.previousClearBlockade != null
        && this.previousClearBlockade.equals(blockadeID)
        && this.previousRepairCost >= 0
        && currentRepairCost >= 0) {
   ...
   *
   * <p>Instead of issuing MOVE from the CLEAR module, change the clear mode and clear direction
   * through the nearest blockade boundary.
   */
  private ActionClear createRecoveryClear(ClearCandidate candidate) {
    ...
    double desiredDistance =
        Math.min(Math.max(0.0, this.clearDistance - 100.0), vectorLength + RECOVERY_EXTENSION);
  ...
  /** Clear previous attempt state when control is handed to another action module. */
  ...
  private ClosestPoint closestPointOnSegment(
